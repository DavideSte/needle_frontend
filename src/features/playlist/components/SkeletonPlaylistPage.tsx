...
export default function SkeletonPlaylistPage() {
  return (
    <div className="m-4 flex flex-col gap-6 lg:mt-8">
      ...
      <div className="flex flex-col gap-4">
        {[...Array(randomIntFromInterval(3, 5))].map((_, index) => {
          return (
            <div key={"artist-" + index}>
              <SkeletonItem className="h-6 w-40 mb-4" />
              <div className="flex flex-col gap-4 lg:grid lg:grid-cols-2">
                {[...Array(randomIntFromInterval(1, 5))].map((_, index) => {
                  return <SkeletonItem key={"album-" + index} className="h-24" />;
                })}
              </div>
            </div>
          );
